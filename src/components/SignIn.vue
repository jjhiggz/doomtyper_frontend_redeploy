<template>
  <div class="nes-container" id="sign-in-wrapper">
    ...
    <h2>OR</h2>
    <h2>Sign Up</h2>
    <form @submit.prevent="signUp" class="sign-in-form">
      <input
        type="text"
        class="username-input"
        placeholder="username"
...
export default {
  ...
  computed: {
    ...
    passwordLength() {
      return this.signupPassword.length;
    },
    passwordValid() {
      return this.passwordLength > 8 ? true : false;
    },
    passMatch() {
      ...
      if (this.passMatch) {
        ...
        fetch(url, requestOptions)
          ...
          .then((result) => {
            const parsedResult = JSON.parse(result);
            if (
              Object.keys(parsedResult).includes("id") &&
              parsedResult.id !== null
            ) {
