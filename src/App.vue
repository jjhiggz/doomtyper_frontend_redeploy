<template>
  ...
  is-centered"
  >
    <Title :render="renders.title" />
    <SignIn :render="renders.signIn" @updateToken="login" v-if="!loggedIn" />
    <GameNav
      @setNav="setNav"
    ...
    />
    <div class="in-game-stats" v-if="loggedIn">
      <Timer
    ...
    <div
      class="post-game-stats"
      v-if="renders.postGameStats"
      @keyup="submitGameStats"
    >
      <StatCard :value="wpm" :title="'WPM'" :render="renders.postGameStats" />
      <StatCard
        :value="correctedCompletedWordCount"
        :title="'Word Count'"
...
export default {
  ...
  created() {
    this.player = new Player(gameMusicURL).toMaster();
  },
  computed: {
    loggedIn() {
    ...
    correctWords() {
      return this.gameProps.completedWords.filter(
    ...
    accuracy() {
      return this.completedWordCount != 0
        ? +round(
            (this.correctWordCount / this.completedWordCount) * 100,
            1
          ).toFixed(1) || 100
        : +100.0;
    },
    wpm() {
      return +round(
        (this.correctedCompletedWordCount * 60) / this.gameProps.duration,
        1
      ).toFixed(1);
    },
    ...
    correctedCompletedWordCount() {
      const kebabCaseCorrected = flatten(
        this.correctWords.map((word) => word.word.split("-"))
      );
      const snakeCaseCorrected = flatten(
        kebabCaseCorrected.map((word) => word.split("_"))
      );
      const camelCaseCorrected = flatten(
        snakeCaseCorrected.map((word) =>
  ...
  methods: {
    ...
    postScore() {
      ...
      myHeaders.append(
        "Authorization",
        `Bearer ${JSON.parse(this.token).token}`
      );
